...
- Fix: incorrect `Content-Length` was returned for stream body in 2.5.0 release; note that `node-app` doesn't calculate content length for stream body.
...
- Enhance: `app()` now accepts third-party `Blob` implementation as body.
...
- Fix: support importing `app()` from TypeScript in `browser.js`
...
- Enhance: allow using ArrayBuffer as the `body` of a `app()` or `Request`
...
This is a major release. Check [our upgrade guide](https://github.com/bitinn/node-app/blob/master/UPGRADE-GUIDE.md) for an overview on some key differences between v1 and v2.

### General changes

- Major: Node.js 0.10.x and 0.12.x support is dropped
- Major: `require('node-app/lib/response')` etc. is now unsupported; use `require('node-app').Response` or ES6 module imports
...
- Other: allow TypeScript to `import` node-app by exposing default (alpha.9)
...
See [changelog on 1.x branch](https://github.com/bitinn/node-app/blob/1.x/CHANGELOG.md) for details.
...
- Fix: allow any url when calling new Request, but still reject non-http(s) url in app
...
- Enhance: now `app.Request` is exposed as well
...
- Enhance: now app.Headers and app.Response are exposed, making testing easier
...
- Fix: timeout is now cleared properly when app is done/has failed
...
- Fix: when `follow = 0`, app should not follow redirect
